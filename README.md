...
新增功能： 可以修改图片在聊天外面显示的内容（把[动画表情]修改为别的）。    
...
如果图片是gif，插件会保留 GIF 格式并以 `图片.gif` 文件发送，保存后可保留动画，想要查看动图需要点击`查看原图`图片才会动起来。  

2.修改图片显示内容：  
回复某图片，发送 */rename [修改的名字]* 即可    
eg： */rename [xb]*  

3.添加群聊黑名单，在指定群聊里面不能使用转换功能（污染了群文件）  
